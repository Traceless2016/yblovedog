...
// https://blog.csdn.net/morainwuhen/article/details/127143860
// NTPClient timeClient(ntpUDP, "ntp.tuna.tsinghua.edu.cn"); //WTP获取时间
...
uint16 ali_cnt = 0;
bool turn_flag = true;
uint32_t max_ntp_consume_time = 0;
...
void update_from_net() {
  uint32_t ntp_consume_time = 0;
  while (!timeClient.update()) {
    delay(3e3);
    Serial.print("*");
    ntp_consume_time++;
    if (ntp_consume_time > 65536) {
      ntp_consume_time = 65536;
    }
    // ntp_consume_time
    max_ntp_consume_time = ntp_consume_time > max_ntp_consume_time ? ntp_consume_time : max_ntp_consume_time;
  }
}

void get_formatted_time_str(String formattedTime) {
  formattedTime = timeClient.getFormattedTime();
  Serial.print("Formatted Time:");
  Serial.println(formattedTime);
}

void get_formatted_time(char *s) {
  String formattedTime = timeClient.getFormattedTime();
  Serial.print("Formatted Time:");
  Serial.println(formattedTime);
  const char *constc = nullptr;    //初始化const char*类型，并赋值为空
  constc = formattedTime.c_str();  //string类型转const char*类型
  // Serial.print(constc);
  // s = const_cast <char*> (constc);  //好像不管用
  if (s) {
    strcpy(s, constc);
  }
...
void wifiInit() {
  ...
  //wifiMulti.addAP("WY-8085", "12345678");
  wifiMulti.addAP("Xiaomi_365F", "15137839141");
  wifiMulti.addAP("18pro", "43426mqj");
  //wifiMulti.addAP("TP-LINK_84FE", "kong$123&K");
...
bool mqttIntervalPost_string(char const *text_str, char const *device_id_name, char const *module_id_name) {
  char param[128];
  char jsonBuf[128];
  ...
  // sprintf(param, "{\"openTime\":\"%s\"}",temp);
  if (device_id_name != "") {
    sprintf(param, "{\"%s:%s\":\"%s\"}", device_id_name, module_id_name, text_str);
  } else {
    sprintf(param, "{\"%s\":\"%s\"}", module_id_name, text_str);
  }
...
bool ali_send_cnt(uint32 cnt, char const *devie_id, char const *module_id, char const *des_cont) {
  /*初始化WiFi，并将cnt发送给阿里云平台*/
  Serial.print(des_cont);
  Serial.println(":");
  Serial.println(cnt);
  // 获取当前时间
  lastMs = millis();
  ...
  Serial.println("mqttIntervalPost_sleep_num!");
  // bool s_flag = mqttIntervalPost_sleep_num(cnt);
  bool s_flag = mqttIntervalPost_int(cnt, devie_id, module_id);
  client.loop();
  return s_flag;
}
...
bool ali_send_running_state(char const *running_state) {
  /*初始化WiFi，并将cnt发送给阿里云平台*/
  Serial.println("ali_send_running_state:");
  ...
  bool s_flag = mqttIntervalPost_string(running_state, "", "RunningState");
  client.loop();
  return s_flag;
}
...
void ali_setup() {
  ...
  // 初始化NTP时间服务
  Serial.print("Start NTP setting!!");
  timeClient.begin();
  update_from_net();  // 迫使从网络获取时间
}
